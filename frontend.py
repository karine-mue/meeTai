...
def session_label(item):
    ...
    return (
        f"{updated} · {title} · {item.get('turns_used') or 0} turns · "
        f"{item.get('llm_calls_used') or 0} calls · {agents}"
    )
...
def archive_params(range_choice, month):
    params = {
        "limit": 100,
        "user_email": st.user.email,
    }
    params["month" if month else "range"] = month or range_choice
    return params
...
def render_archive():
    with st.expander("ARCHIVE / past sessions", expanded=False):
        ...
        if range_choice == "month":
            ...
            if not month:
                st.markdown(
                    "<div style='color:#555;font-size:12px;padding:8px 0'>年月を YYYY-MM で入力</div>",
                    unsafe_allow_html=True,
                )
                return

        try:
            sessions = get("/sessions", params=archive_params(range_choice, month)).json().get("sessions", [])
        ...
        if not sessions:
            st.markdown(
                "<div style='color:#333;font-size:12px;padding:8px 0'>no archived sessions</div>",
                unsafe_allow_html=True,
            )
            return
        ...
        selected_idx = st.selectbox(
            ...
            format_func=lambda i: session_label(sessions[i]),
        ...
        st.markdown(
            f"<div class='archive-row'>selected: {html.escape(title)} · {html.escape(sid)}</div>",
            unsafe_allow_html=True,
        )
        ...
        try:
            md = get(
                ...
                params={"format": "markdown", "user_email": st.user.email},
                timeout=15.0,
            ).text
            js = json.dumps(
                get(
                    f"/sessions/{sid}/export",
                    params={"format": "json", "user_email": st.user.email},
                    timeout=15.0,
                ).json(),
                ensure_ascii=False,
                indent=2,
...
if not st.session_state.session_id:
    st.markdown(
        "<div style='color:#555;font-size:12px;margin-bottom:12px'>参加エージェントを選択してセッションを開始</div>",
        unsafe_allow_html=True,
    )
...
if view in [a.upper() for a in AGENTS]:
    ...
    st.markdown(
        "<div style='font-size:10px;color:#444;letter-spacing:.1em;margin-bottom:8px'>LATEST TURN</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div style='font-size:10px;color:#444;letter-spacing:.1em;margin-bottom:8px'>MINUTES（全ログ）</div>",
        unsafe_allow_html=True,
    )
